...
class SpeedTestConfig:
    ...
    vocab_size = 30522
    padding_idx = 0
    max_position_embeddings = 100 + 1 #L
    pad_position = 0
    num_token_types = 3
    pad_token_type = 0
...
def main():
	#nub_heads = [2,8,16,64]
	nub_heads = [2,4,8,16]
	test_max_position_embeddings = np.array([64,128,256])
	test_embedding_dim = np.array([16,32,64])
	...
	for i in range(length(nub_heads)):
		for j in range(length(test_max_position_embeddings)):
			for k in range(length(test_embedding_dim)):
				...
				for _ in range(nb_episodes):
					start = time.time()
					test_mha_emb, test_mha_filters = test_togepi_mha_obj(test_emb)
					end = time.time()
